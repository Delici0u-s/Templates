...
def multiselect(message: str, choices: Sequence[str], *, preselected: Sequence[str] = ()) -> list[str] | None:
    """Pick several items. Returns None when cancelled."""
    options = list(choices)
    ...
    print(f"{message}  (comma-separated numbers, empty to cancel)")
    for index, option in enumerate(options, 1):
