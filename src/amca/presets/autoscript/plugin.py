...
class autoscript(Plugin):
    ...
    def load(self, ctx: PluginContext) -> int:
        opts, forwarded = _parse(ctx.args)
...
def _parse(args: list[str]) -> tuple[argparse.Namespace, list[str]]:
    """Split our own flags from what gets forwarded to the script.

    ``parse_known_args`` rather than a full parser: everything we do not
    recognise belongs to the script, including ``--help``.
    """
